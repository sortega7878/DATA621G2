...
```
```{r su_1, echo=FALSE}
# Just looking at variables with missing values
descr1 <- describe(td1)
row.names(descr1[descr1$n != 2276,])
descr1[descr1$n != 2276,"n"]
```
...
```{r su_2}
# Some plots which may help for finding Transformations and Collerations

# pair plots for all variables 
windows()
pairs(td2, gap = 0.5)

# plots for individual pairs
windows()
plot(TEAM_FIELDING_E ~ TARGET_WINS, data = td2)
# plot after some transformation
windows()
plot(log(TEAM_FIELDING_E) ~ TARGET_WINS, data = td2)

windows()
plot(TEAM_PITCHING_BB^2 ~ TARGET_WINS, data = td2)
windows()
plot(log(TEAM_PITCHING_BB) ~ TARGET_WINS, data = td2)

windows()
plot(TEAM_BATTING_HR ~ TARGET_WINS, data = td2)
windows()
plot(sqrt(TEAM_BATTING_HR) ~ TARGET_WINS, data = td2)

#td2$TEAM_FIELDING_E <- log(td2$TEAM_FIELDING_E)
#td2$TEAM_BATTING_HR <- sqrt(td2$TEAM_BATTING_HR)
#td2$TEAM_PITCHING_BB <- log(td2$TEAM_PITCHING_BB)
#lm(medv~lstat +I(lstat^2), Boston)
...
```{r su_3}
# Example of adding tranformed variable to the model

#mod1a <- lm(TARGET_WINS ~ . -TEAM_FIELDING_E + I(log(TEAM_FIELDING_E)), data=td2)
mod1a <- lm(TARGET_WINS ~ . +I(log(TEAM_PITCHING_BB)), data=td2)
summary(mod1a)
...
```{r su_3.1}
# Example of adding iterraction (combination) of two variables to the model

mod1a <- lm(TARGET_WINS ~ . +TEAM_BATTING_HR*TEAM_PITCHING_BB, data=td2)
summary(mod1a)
...
```{r su_3.2}
# Just adding combined term and checking the impact on the model

mod1b <- lm(TARGET_WINS ~ . -TEAM_BATTING_HR-TEAM_BATTING_SO-TEAM_FIELDING_E+TEAM_BATTING_HR*TEAM_PITCHING_BB, data=td2)
summary(mod1b)

mod1c <- lm(TARGET_WINS ~ . -TEAM_BATTING_HR-TEAM_BATTING_SO-TEAM_FIELDING_E-TEAM_BATTING_2B+TEAM_BATTING_HR*TEAM_PITCHING_BB, data=td2)
summary(mod1c)
```
